...
class DownloadWorker(
    ...
    // Attempt kab shuru hua — first-progress deadman ke liye (LIVELOCK fix 2026-07-16:
    // TikTok vt.* links pe yt-dlp retry-loop me kabhi-kabhi output deta rehta tha →
    // silence-watchdog pacified, par progress 0% se kabhi na hila → "starting…" pe
    // ghanton latka + slot qabza. 15 min tak koi % nahi = link dead → kill/retry.)
    ...
    override suspend fun doWork(): Result {
        ...
        val wake = try {
            (ctx.getSystemService(Context.POWER_SERVICE) as? PowerManager)
                ?.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Riplox:dl")?.also {
        ...
        val wifi = try {
            (ctx.applicationContext.getSystemService(Context.WIFI_SERVICE) as? WifiManager)
                ?.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "Riplox:dl")?.also {
                    it.setReferenceCounted(false)
                    it.acquire()
                }
        } catch (e: Exception) {
        ...
        return try {
            ...
            coroutineScope {
                ...
                // chale" wala masla). Escort-FGS ya app/activity khulte hi agli retry pe
                // REAL FGS lag jati hai → wahan se download app band hone pe bhi chalti.
                        ...
                        val watchdog = launch(Dispatchers.IO) {
                            while (true) {
                                ...
                                if (now - lastBeat > limit || neverStarted) {
                                    Log.w("XniperDL", "watchdog: ${if (neverStarted) "no progress ${FIRST_PROGRESS_MS / 1000}s" else "no output ${limit / 1000}s"} — killing $pid")
                                    stalled = true
                                    try { YoutubeDL.getInstance().destroyProcessById(pid) } catch (_: Throwable) {}
                                    dl.cancel()
                                    break
        ...
        try {
            WorkManager.getInstance(activity.applicationContext)
                ...
                .observe(activity) { info ->
                    ...
                    // FAILED/CANCELLED pe "started" TRUE mat bolo (galat toast jata tha).
