 ...
 *
 * Masla jo ye hal karta hai (2026-07-16): door 10s me band ho jata tha — agar WM job
 * us window me RUNNING+fg tak na pahunchi (JobScheduler defer / bucket / XOS), to app
 ...
 * ⚠️ setExpedited YAHAN BHI KABHI NAHI (2026-07-09 lesson — quota job, FGS nahi).
 */
class EscortService : Service() {
    ...
    override fun onCreate() {
        ...
        try {
            ...
            Log.w("XniperDL", "escort startForeground fail", e)
            stopSelf()
            return
        }
        watch()
    ...
    companion object {
        ...
        fun start(ctx: Context) {
            try {
                ...
                Log.w("XniperDL", "escort start denied (bg?)", e)
            }
        }

        /** App/Downloads-page khulne pe: koi phansi/pending job ho to escort se
         * dhakka do (stale ENQUEUED jobs ka "app open pe bhi start nahi" fix). */
